---
title: "HW 01 - Gotta catch 'em all"
output: 
  html_document: 
    theme: yeti
    css: hw.css
...
knitr::include_graphics("img/hw-01-pikachu.png")
```
</div>
...
- Start with an assignment link that creates a repo on GitHub with starter documents: [UPDATE INSTRUCTIONS WHEN `ghclass` CAN CREATE INDIVIDUAL ASSIGNMENTS]
...
1. Load the data from the csv file provided: `pokemon.csv`.

2. Calculate the diference in heights pre and post evolution and save this as a new variable. Calculate the percentage of pokemon that grew during evolution. Also visualize the distribution of change in height by species and provide a discussion of how change in height varies across species.

3. Recreate the following plot.

![recreate](img/hw-01-recreate.png)

4. Calculate the relative frequency of `attack_strong_new` becased on levels of `attack_strong`. That is, the relative frequency of post-evolution stronger attack based on the pre-evolution stronger attack. Do Pokemon tend to change their strong attack post-evolution?

5. Pick two categorical variables and make a bar plot that depicts the relationship between them. These can be variables from the original data or ones that you create based on the given data.

6. Pick a numerical and a categorical variable, and construct side-by-side box plots depicting the relationship between them.
...
8. What characteristics correspond to an evolved Pokémon with a high combat power? You do not need to come up with an exhaustive list, but you should walk us through your reasoning for answering this question and include all relevant summary statistics and visualizations.
...
**A note on sharing / reusing code:** I am well aware that a huge volume of code is available on the web to solve any number of problems. Unless I explicitly tell you not to use something the course's policy is that you may make use of any online resources (e.g. StackOverflow) but you must explicitly cite where you obtained any code you directly use (or use as inspiration). Any recycled code that is discovered and is not explicitly cited will be treated as plagiarism. On individual assignments you may not directly share code with another student in this class. You are welcome to discuss the problems with class mates and ask for advice, but you may not send or make use of code from others.

## Grading

Total                                                   | 100 pts
--------------------------------------------------------|--------
Questions 1-7                                           | 60 pts
Question 8                                              | 20 pts
